...
const Restore = () => {
  const { showTimeSoft, removeShowtimeSoft, removeShowtime, restoreShowtime } = useContext(ContextMain)
  ...
  const handleConfirmRestore = () => {
    if (confirmRestoreId) {
      restoreShowtime.mutate(confirmRestoreId)
        .then(() => {
          toast.success('Xóa lịch chiếu thành công');
        })
        .catch((error:any) => {
              ...
              <tr className="hover:bg-gray-50">
                <th className="flex gap-3 px-6 py-4 font-normal text-gray-900">
                  <div className="text-sm">
                    <div className="font-medium text-gray-700">{item?.screenRoomId.name}</div>
                  </div>
                </th>
              ...
              {item?.status}
                ...
                <td className="px-6 py-4">
                  <div className="flex justify-between items-center gap-4">
                    ...
                    <TbRestore className='text-2xl cursor-pointer' 
...
{conFirmRestore && (
  <div className="bg-slate-800 bg-opacity-50 flex justify-center items-center absolute top-0 right-0 bottom-0 left-0">
  <div className="bg-white px-16 py-14 rounded-md text-center">
    <h1 className="text-xl mb-4 font-bold text-slate-500">Bạn có muốn khôi phục không</h1>
    <button className="bg-red-500 px-4 py-2 rounded-md text-md text-white" onClick={() => setConFirmRestore(false)}>Cancel</button>
    <button className="bg-indigo-500 px-7 py-2 ml-2 rounded-md text-md text-white font-semibold" onClick={handleConfirmRestore}>Ok</button>
  </div>
</div>
        )}
          </tbody>
