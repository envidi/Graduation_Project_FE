...
import { API_GOOGLE, SIGN_IN_GOOGLE } from '@/utils/constant'
...
function LoginWithGoogle() {
  ...
  const handleLoginGoogleSucess = async (credentialResponse: {
    access_token: string
  }) => {
    const { data: credential } = await axios.get(API_GOOGLE, {
      headers: {
  ...
  const login = useGoogleLogin({
    onSuccess: (credentialResponse) =>
      handleLoginGoogleSucess(credentialResponse),
    onError: handleLoginGoogleError
  })
  return (
    <button
      className="flex w-full items-center justify-center gap-3.5 rounded-lg border border-stroke bg-gray p-4 hover:bg-opacity-50 dark:border-strokedark  dark:hover:bg-opacity-50"
      onClick={() => login()}
    >
      <span>
        <svg
